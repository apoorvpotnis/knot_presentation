...
% 				\end{tikzpicture}\label{fig:braidinverse4}}
	...
	\begin{frame}
		...
		\begin{figure}
			...
			\caption*{A type II move illustrating \(\sigmaa_i\hspace{1pt} \sigmaa_i^{-1} = \I_n\)}
			\label{fig:type2}
		\end{figure}
	\end{frame}
	...
	\begin{frame}
		...
		\begin{figure}\centering
			...
			\caption*{A type III move illustrating \(\sigmaa_i\hspace{1pt} \sigmaa_{i+1} \sigmaa_i = \sigmaa_{i+1} \sigmaa_i\hspace{1pt} \sigmaa_{i+1}\)}
			\label{fig:type3}
		\end{figure}
	\end{frame}
	...
	\begin{frame}
		...
		\begin{figure}\centering
			...
			\caption{Sliding of crossings illustrating \(\sigmaa_i\hspace{1pt} \sigmaa_j = \sigmaa_j\hspace{1pt} \sigmaa_i\)}
			\label{fig:sliding}
		\end{figure}
	\end{frame}
...
% 			\caption*{Generators \(\sigmaa_i\) and \(\sigmaa_i^{-1}\)}
	...
	\begin{frame}
		\frametitle{Normalised bracket polynomial \(L(K)\)}\vspace{10pt}
...
% 	\section{Markov trace}
	...
	\begin{frame}
		...
		\begin{itemize}
			...
			\item the free additive algebra \(\TL_n\) (as a module)
	...
	\begin{frame}
		...
		Louis Kauffman's book on which this presentation is based:\vspace{10pt}
	...
	\begin{frame}
		...
		Original papers by Emil Artin and Frederic Bohnenblust:\vspace{10pt}
		\begin{enumerate}
			\item \fullcite{artinzopfe}

			This paper is in German. I am not aware of an English translation. It contains some errors which have been corrected in the 1947 paper by Artin.

			\item \fullcite{artin}
			\item \fullcite{bohnenblust}
		\end{enumerate}
	\end{frame}

	\begin{frame}
	    \frametitle{References (contd.)}
		General reference books for knot theory and braids:\vspace{10pt}
		\begin{enumerate}
			\item \fullcite{murasugibraids}
			\item \fullcite{cromwell}
			\item \fullcite{birman}

			This book contains the first proof of the Markov theorem, based on the notes of J.\@ H.\@ Roberts and an unknown speaker at Princeton University.
		\end{enumerate}
	\end{frame}
	...
	\begin{frame}
		\frametitle{References (contd.)}
		\begin{enumerate}
			\item \fullcite{markov}

			This paper is in German and contains a sketch of a proof.
			\item \fullcite{alexander}

			This paper introduced and proved the now known Alexander's theorem.
			\item \fullcite{morton}

			This paper contains a shorter proof of the Markov theorem.
		\end{enumerate}
	\end{frame}
	...
	\begin{frame}
		\frametitle{References (contd.)}
		\begin{enumerate}
			\item \fullcite{temperley-lieb}

			This paper introduces the Temperley--Lieb algebras in a statistical physics context.
			\item \fullcite{jones}

			This paper introduces the Jones polynomial.
		\end{enumerate}
