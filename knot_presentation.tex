...
\usepackage{tikz, tikz-3dplot, tikz-cd, adjustbox, subcaption}
...
% 	\end{frame}
	...
	\begin{frame}{Geometric definition of braids}
		\begin{figure}
		    \centering
			\begin{tikzpicture}
				[scale=1.2,
...
% 				\node[left, label={O}] at (-0.05,-0.1,0) {};
	...
	\begin{frame}
		...
		\begin{figure}\centering
			\begin{tikzpicture}[scale=1]
				\begin{knot}[yscale=2, clip width=5]
...
% 					\flipcrossings{2}
	...
	\begin{frame}
		...
		\begin{figure}
			...
			\subcaptionbox*{\(b^{-1}\)}{
				\begin{tikzpicture}[scale=1]
					\begin{knot}[clip width=5, yscale=-1]
						\strand[thick, red] (1,0) to [in angle=-90, out angle=90, curve through = {(1.5,0.5)}] (1,2);
						\strand[thick, red] (2,0) to [in angle=-90, out angle=90, curve through = {(1.9,0.5)}] (3,2);
						\strand[thick, red] (3,0) to [in angle=-90, out angle=90, curve through = {(2,1.3)}] (2,2);
						\flipcrossings{2}
...
% 					\begin{knot}[clip width=5, yscale=2]
% 						\strand[thick, red] (1,0) to (1,2);
% 						\strand[thick, red] (2,0) to (2,2);
% 						\strand[thick, red] (3,0) to (3,2);
% 						% \flipcrossings{2}
...
\[F = ma ABCDEFGHIJKLMNOPQRSTUVWXYZ\]
\[\int \oint \sum\]
...
\begin{frame}{Mapping theorem}
	\begin{thm}[Mapping]
		If \(\tilde{f} \in \homeo_\kappa^*(\tilde{\sigma}, \tilde{z}) \cap \homeo_0(\tilde{\sigma}, \tilde{z})\), then \[\kappa_*(\tilde{f}\hspace{0.5pt}) = f \in \homeo_0(\sigma, z).\] Moreover, the induced homomorphism \[\kappa_* \colon \homeo_\kappa^*(\tilde{\sigma}, \tilde{z}) \cap \homeo_0(\tilde{\sigma}, \tilde{z}) \rightarrow \homeo_0(\sigma, z)\] is an isomorphism.
	\end{thm}
	\begin{proof}
		Obvious.
	\end{proof}
